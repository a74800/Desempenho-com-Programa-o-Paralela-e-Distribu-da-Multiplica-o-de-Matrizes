...
programa="./mult_matrix"  # Altera para o nome correto do teu executável
python_script="gera_matrix.py"

# Loop externo para variar o tamanho da matriz como potências de 2 (2^N), de N=7 até N=11
for N in $(seq 8 11); do
    ...
    echo "Execução,Tempo_real, Tempo_user, Tempo_sys" > $output_file
    ...
    # Loop para executar 5 vezes (ignora a primeira)
    for i in $(seq 1 30); do
        # Executa o programa e mede o tempo com `time`, capturando apenas "real"
        tempos=$( { time $programa $tamanho > /dev/null; } 2>&1 )
        ...
        if [ $? -ne 0 ]; then
            echo "As matrizes são diferentes para tamanho $tamanho. Interrompendo a execução."
            exit 1
        fi

        # Extrai os tempos corretos
        ...
        # Converte o tempo para segundos (removendo 'm' e 's' do formato mm:ss)
        ...
        echo "$i,$tempo_real,$tempo_user,$tempo_sys" >> $output_file
       ...
       # echo "Execução $i: real $tempo_real, user $tempo_user, sys $tempo_sys"
    done
    ...
    echo "Média,$media_real,$media_user,$media_sys" >> $output_file
    echo "Total,$total_real,$total_user,$total_sys" >> $output_file
...
echo "Última média do tempo de execução: real=$media_real, user=$media_user, sys=$media_sys"
